...
def img_feat_proj(coords, feats, image, calib):
    """
    Project image features to point cloud.
    Args:
        coords (np.ndarray, shape=[N, 3]): Coordinates of points.
        feats (np.ndarray, shape=[N, 4]): Features of points.
        image (np.ndarray, shape=[H, W, 3]): Image.
        calib (dict): Matrix to project points in camera coordinate to lidar coordinate.
    Returns:
        feats_uvrgb: np.ndarray, shape=[N, 5]: points with image features(uvrgb).
    """
